        ...
        for code in self.code_list:
            ...
            for i in xrange(shrink_period + 1, date_length - 1):
                ...
                if count == shrink_period - 1:  # 连续缩量备选；
                    ...
                    if self.all_data[code]['open'][i] > self.all_data[code]['close'][i-1] and \
                        self.all_data[code]['low'][i] > self.all_data[code]['close'][i-1] and \
                        self.all_data[code]['volume'][i] > self.all_data[code]['volume'][i-1]:
                        # 获取后一天的开盘涨幅，收盘涨幅：
                        p_profit_open = (self.all_data[code]['open'][i+1] - self.all_data[code]['close'][i])/self.all_data[code]['close'][i]
                        p_profit_open = round(100 * p_profit_open, 2)
                        p_profit_close = (self.all_data[code]['close'][i+1] - self.all_data[code]['close'][i])/self.all_data[code]['close'][i]
...
if __name__ == "__main__":
    ...
    F = open('algorithm_2_result_file', 'w')
